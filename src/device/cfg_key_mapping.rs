...
pub fn build_cfg_vals_from_config(config: &UbloxConfig) -> Vec<CfgVal> {
    ...
    // Rate settings (derived from `device.navigation.rate_hz`; nav_ratio fixed
    // at 1 since F9P RTK requires a PVT solution every measurement epoch).
    vals.push(CfgVal::RateMeas(config.measurement_ms));
    vals.push(CfgVal::RateNav(1));
