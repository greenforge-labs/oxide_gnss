...
pub use ublox::{
    ...
    ProtocolConfig, QzssConfig, SbasConfig, SignalConfig, SurveyInConfig, TimeGrid, TimeMarkConfig,
...
impl Config {
    ...
    ///
    /// `measurement_ms` is always derived from `device.navigation.rate_hz` here,
    /// regardless of path — there is one authoritative rate knob.
    pub fn resolve_ublox_config(&self) -> UbloxConfig {
        let mut cfg = if let Some(mode) = self.mode {
    ...
    fn generate_ublox_config_from_mode(&self, mode: OperatingMode) -> UbloxConfig {
        ...
        // `measurement_ms` is populated by `resolve_ublox_config` from
        // `device.navigation.rate_hz` — leave it at the Default value here
        ...
        UbloxConfig {
            ...
            measurement_ms: UbloxConfig::default().measurement_ms,
            protocols,
            messages,
            ports,
            signals,
...
"#;
    ...
    // `navigation.rate_hz` is the single authoritative rate knob and must
    // land on `UbloxConfig::measurement_ms` as 1000 / rate_hz.
    #[test]
...
"#;
        ...
        assert_eq!(ublox.measurement_ms, 200);
    }

    #[test]
    fn test_rate_hz_derives_measurement_ms_legacy_mode() {
        let yaml = r#"
device:
  port: "/dev/ttyACM0"
  navigation:
...
"#;
        let config: Config = serde_yaml::from_str(yaml).unwrap();
        let ublox = config.resolve_ublox_config();
        assert_eq!(ublox.measurement_ms, 100);
    }
