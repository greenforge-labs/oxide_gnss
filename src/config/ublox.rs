...
pub struct UbloxConfig {
    ...
    /// Measurement period in milliseconds, derived from `device.navigation.rate_hz`.
    ///
    /// Not user-configurable via YAML — `Config::resolve_ublox_config` populates
    /// this from `navigation.rate_hz` so there is one authoritative rate knob.
    #[serde(skip, default = "default_measurement_ms")]
...
impl Default for UbloxConfig {
    fn default() -> Self {
        Self {
            family: None,
            measurement_ms: default_measurement_ms(),
