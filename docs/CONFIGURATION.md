...
This document is the exhaustive parameter reference for oxide_gnss. For worked examples and task-oriented walkthroughs, see [USER_MANUAL.md](USER_MANUAL.md); for integrity thresholds see [INTEGRITY.md](INTEGRITY.md); for topics see [TOPICS.md](TOPICS.md).
...
| `integrity` | Jamming/spoofing detection, protection levels | SEC_SIG, MON_RF, MON_COMMS, NAV_SAT, NAV_PL | ~/integrity, ~/operational |
...
`rate_hz` is the single rate knob: the driver writes `CFG-RATE-MEAS = 1000 / rate_hz` and `CFG-RATE-NAV = 1` (one PVT solution per measurement, as F9P RTK requires). See **Navigation rate vs constellations** further down for the realistic rates each mode can sustain and how to tune the trade-off.
...
Set via `device.navigation.rate_hz` (see **Navigation Settings** above). The driver writes `CFG-RATE-MEAS = 1000 / rate_hz` and hardcodes `CFG-RATE-NAV = 1`.
...
| Mode | `rate_hz` | Why |
|---|---|---|
| `standalone` | 10 | Light workload — just NAV-PVT |
| `rover_ntrip` | 5 | All constellations + integrity + RTCM decode — F9P ceiling |
| `rover_radio` | 5 | Same as `rover_ntrip` |
| `moving_base` | 5 | Heavy — computes the base solution + RTCM output |
| `moving_base_rover` | 5 | Matches the moving base |
| `static_base` | 1 | Stationary — rate is irrelevant |
...
1. **Rate** — `device.navigation.rate_hz`
   ```yaml
   device:
     navigation:
       rate_hz: 10   # request 10 Hz
   ```
...
- **Europe, 10 Hz rover_ntrip:** keep GPS + Galileo dual-band on, disable GLONASS + BeiDou + QZSS, set `rate_hz: 10`.
...
- **High-dynamics GPS-only survey at 20 Hz:** disable all non-GPS constellations and integrity feature, set `rate_hz: 20`.
