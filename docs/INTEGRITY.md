...
> The values above are the u-blox MON-RF protocol codes. The `antenna_status` field exposed on the `~/integrity` ROS message uses a separate, smaller encoding (`0=Unknown, 1=Ok, 2=Open, 3=Short`); see [§7.5](#75-securityhardware-fields).
...
| `tmir` | `> max_tmir_per_epoch` (default: 6.0, in %MI/epoch) | → **DEGRADED** ("TMIR exceeds threshold") |
...
integrity:
  thresholds:
    ...
    jitter_grace_ms: 200            # Extra slack for async processing jitter
    ...
    max_tmir_per_epoch: 6.0         # %MI/epoch; above this → DEGRADED
