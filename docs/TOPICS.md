...
Two QoS profiles are used:

| Profile | Reliability | History | Used For |
|---------|-------------|---------|----------|
| Sensor data | Best-effort | Keep last | High-rate sensor streams where latest sample matters (`~/fix`, `~/velocity`, `~/baseline_pose`) |
| Reliable | Reliable | Keep last | Safety-relevant topics that must not be dropped (`~/integrity`, `~/operational`), and default-QoS topics (`~/time_reference`, `/diagnostics`, `~/satellites`) |
...
- **UBX Messages:** `NAV_PVT`, `SEC_SIG`, `MON_RF`, `MON_COMMS`, `NAV_SAT`, `NAV_PL`
...
| Topic | Type | QoS | Required UBX | Optional UBX |
|-------|------|-----|--------------|--------------|
| `~/fix` | `sensor_msgs/NavSatFix` | Sensor | `NAV_PVT` | `NAV_HPPOSLLH` |
| `~/velocity` | `geometry_msgs/TwistWithCovarianceStamped` | Sensor | `NAV_PVT` | – |
| `~/time_reference` | `sensor_msgs/TimeReference` | Reliable (default) | `NAV_PVT` | – |
| `/diagnostics` | `diagnostic_msgs/DiagnosticArray` | Reliable (default) | – | `NAV_SVIN` |
| `~/integrity` | `oxide_gnss_msgs/OxideIntegrity` | Reliable | `NAV_PVT`, `SEC_SIG`, `MON_RF`, `MON_COMMS`, `NAV_SAT`, `NAV_PL` | `SEC_SIGLOG`, `RXM_COR`, `NAV_COV` |
| `~/operational` | `std_msgs/Bool` | Reliable | (same as `~/integrity`) | – |
| `~/satellites` | `oxide_gnss_msgs/OxideSatellites` | Reliable (default) | `NAV_SAT` | – |
| `~/baseline_pose` | `geometry_msgs/PoseWithCovarianceStamped` | Sensor | `NAV_RELPOSNED` | – |
