...
|------------|---------|----------|
| ROS 2 environment | ROS 2 framework — either a system install (Humble/Jazzy/Kilted) or a [pixi/RoboStack](https://robostack.github.io) workspace | [ROS 2 install](https://docs.ros.org/en/jazzy/Installation.html) or `pixi init` + `pixi add ros-jazzy-desktop` |
| Rust (stable) | Compiler | `curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs \| sh` |
| libclang-dev | Bindgen (FFI) | `sudo apt install libclang-dev` (skip under pixi — provided by env) |
...
With pixi/RoboStack (the env provides its own message packages, so `--allow-overriding` is not needed):

```bash
cd ~/ros2_ws
pixi shell   # activates ROS 2 and auto-sources install/setup.bash via pixi.toml
colcon build --packages-up-to oxide_gnss_msgs oxide_gnss
```

This creates `.cargo/config.toml` with patches redirecting ROS 2 crates to locally-built versions.
...
With a system ROS 2 install, source in this order:

```bash
source /opt/ros/jazzy/setup.bash      # 1. ROS 2 base
source ~/ros2_ws/install/setup.bash   # 2. Workspace overlay
```

Under pixi/RoboStack, `pixi shell` does both (the ROS env is activated, and `pixi.toml` auto-sources the workspace overlay). No manual sourcing needed.
...
# Rebuild from scratch (system ROS 2)
source /opt/ros/jazzy/setup.bash
colcon build --packages-up-to oxide_gnss

# Or with pixi:
# pixi shell && colcon build --packages-up-to oxide_gnss
```
